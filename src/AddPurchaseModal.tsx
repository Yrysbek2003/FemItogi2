...
export const AddPurchaseModal: React.FC<AddPurchaseModalProps> = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Добавить закупку" size="lg">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {formData.quantity && formData.costPerUnit && (
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl">
            <p className="text-sm text-blue-700 dark:text-blue-300">
              <strong>Общая стоимость: {(Number(formData.quantity) * Number(formData.costPerUnit)).toLocaleString()} сом</strong>
            </p>
          </div>
        )}
        ...
        <div className="flex gap-4 pt-4">
          <button
            type="submit"
            className="flex-1 bg-gradient-to-r from-purple-600 to-purple-700 text-white py-3 px-6 rounded-xl hover:from-purple-700 hover:to-purple-800 transition-all duration-200 font-medium"
          >
            Добавить закупку
          </button>
          <button
